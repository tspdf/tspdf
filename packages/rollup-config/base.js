...
/**
 * Creates an enhanced external dependency checker function
 * @param {Array} defaultExternal - Array of external dependencies
 * @param {Object} options - Additional options
 * @param {boolean} options.nodeBuiltins - Whether to externalize Node.js built-ins (default: true)
 * @param {Array} options.additionalExternals - Additional externals to check
 * @param {Array} options.bundledDependencies - Dependencies that should be bundled instead of externalized
 * @returns {Function} External dependency checker function
 */
export function createExternalChecker(defaultExternal = [], options = {}) {
  const {
    nodeBuiltins = true,
    additionalExternals = [],
    bundledDependencies = [],
  ...
  const nodeBuiltinModules = nodeBuiltins
    ? [
        'assert',
        'buffer',
        'child_process',
  ...
  const allExternals = [
    ...defaultExternal.filter(dep => !bundledDependencies.includes(dep)), // Exclude bundled dependencies
    ...additionalExternals,
    ...nodeBuiltinModules,
  ];

  return id => {
    // Always bundle dependencies specified in bundledDependencies
    if (
    ...
    if (allExternals.includes(id)) return true;

    // Check for scoped package subpath imports (e.g., "@babel/core/package.json")
    if (allExternals.some(dep => id.startsWith(`${dep}/`))) return true;

    // Check for Node.js protocol imports (node:fs, node:path, etc.)
    if (nodeBuiltins && id.startsWith('node:')) return true;
